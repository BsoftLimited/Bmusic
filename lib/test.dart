...
class Test extends StatefulWidget {
  @override
  State<Test> createState() => __TestState();
}

class __TestState extends State<Test> {
  ...
  /*final list = [
    {
      ...
      "url": "files/All Falls Down.mp3",
      "coverUrl": "files/ic_splash.png"
    }
  ];*/
  ...
  void loadFile() async {
    ...
    final audioFile = await rootBundle.load("files/All Falls Down.mp3");
    final audio = audioFile.buffer.asUint8List();

    final appDocDir = await getApplicationDocumentsDirectory();
    print(appDocDir);

    final file = File("${appDocDir.path}/All Falls Down.mp3");
    file.writeAsBytesSync(audio);

    AudioInfo info = AudioInfo("file://${file.path}",
        title: "file", desc: "local file", coverUrl: "files/ic_splash.png");
        ...
        body: Center(
          child: Column(
            children: <Widget>[
              ...
              Center(
                  child: Text(_error ?? "${AudioManager.instance.info?.title} lrc text: $_position")),
              bottomPanel()
            ],
          ),
        ),
  ...
  Widget songProgress(BuildContext context) {
    ...
    return Row(
      children: <Widget>[
        Text(_formatDuration(_position),style: style,
        ),
        Expanded(
          child: Padding(
            padding: EdgeInsets.symmetric(horizontal: 5),
            child: SliderTheme(data: SliderTheme.of(context).copyWith( trackHeight: 2, thumbColor: Colors.blueAccent, overlayColor: Colors.blue,
